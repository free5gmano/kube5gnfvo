...
# NSLifecycleSubscriptions 模組測試 - 運行所有 API 測試

BASE_URL="${1:-http://localhost:8000}"
PASSED=0
FAILED=0

echo "🧪 NSLifecycleSubscriptions 測試"
echo "---"

apis_dir="$(dirname "$0")/apis"

# 獲取 NS Instance ID
NS_LIST=$(curl -s -X GET "$BASE_URL/nslcm/v1/ns_instances/")
NS_ID=$(echo "$NS_LIST" | grep -o '"id":"[^"]*' | head -1 | cut -d'"' -f4)

if [ -z "$NS_ID" ]; then
  echo "⚠️  沒有找到 NS Instance，跳過測試"
  exit 0
fi
...
# 2. 建立訂閱
bash "$apis_dir/03-create-subscription.sh" "$BASE_URL" "$NS_ID" > /tmp/test_output.txt 2>&1
if [ $? -eq 0 ]; then
