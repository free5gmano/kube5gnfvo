...
class DestinationRuleClient(KubernetesApi):
    ...
    def check_resource(self, **kwargs):
        spec_info = { "host": self.name_of_service if self.name_of_service else self.instance_name}
        if "canary" in self.specific_info:
            subsets = list()
            specific = self.specific_info['canary']
            for hostname, version in zip(specific['hostname'], specific['subset']):
                subset_value = {
                    "name": hostname, 
                    "lables": {
                        "version": version
                ...
                subsets.append(subset_value)
            spec_info['subsets'] = subsets
        if "circuit_breaking" in self.specific_info:
            specific = self.specific_info['circuit_breaking']
            spec_info['trafficPolicy'] = {
                "connectionPool": {
                    "tcp": {
                        "maxConnections:": specific['maxConnections']
                    },
                    "http": {
                        "http1MaxPendingRequests": specific['maxRequestsPerConnection'],
                        "maxRequestsPerConnection": specific['maxRequestsPerConnection']
                    }
                }
            }
        return spec_info
